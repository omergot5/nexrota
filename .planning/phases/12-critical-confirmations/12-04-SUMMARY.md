...
- **Live-browser verification complete (2026-09-24, performed by the orchestrating session)**, using a fresh disposable test team (security-mode, code QEGUQF, cleaned up afterward):
  1. **CTA publish button:** Seeded 14 shifts via `fillWeek` (unpublished by default, unlike demo-seeded shifts which start pre-published), ran smart-assign so the CTA wasn't disabled by "no one assigned yet", then navigated to the wizard's step 4 CTA — that one turned out to be a plain nav link to step 5, not the publish action itself. The actual gated CTA is the "שלח לצוות" button at the bottom of step 5 (`ScheduleMgmt`, embedded inside `WeekFlow`) — separate from `ScheduleMgmt`'s own top "פרסם הכל" button already verified in 12-03. Clicking it opened `ConfirmDialog` ("לשלוח את הסידור לצוות? 14 משמרות ייראו מיד..."); confirming published all 14 shifts immediately (14/14) with no UndoBar, and the CTA changed to "הסידור אצל הצוות".
  2. **Demo-cleanup button:** Inserted one `is_demo=true` shift directly via SQL (the UI's demo-seed trigger no longer appears once team onboarding is complete) alongside the 14 real, already-published shifts. The "מחק נתוני הדגמה לשבוע זה" button correctly appeared on the board step only once the demo shift existed. Clicking it opened `ConfirmDialog` ("למחוק את נתוני ההדגמה של השבוע? 1 רשומות הדגמה יימחקו... שיבוצים אמיתיים... לא ייפגעו"); confirming removed exactly the demo shift, immediately, no UndoBar — confirmed via direct SQL that all 14 real (non-demo) shifts and their assignments survived untouched, and the button disappeared once no demo data remained.
- **Known, intentionally untouched:** The "ביטול הפצה" (unpublish) bug (STATE.md Blockers, 12-CONTEXT.md finding §2, Phase 13 scope) was not exercised by this plan's CTA button, since it only ever calls `publish(ids, true)` (never `false`) — not directly reachable through this specific button regardless.
