...
- **Live-browser verification complete (2026-09-25, performed by the orchestrating session)**, using a fresh disposable army-mode test team (code 2PYBTY, cleaned up afterward):
  1. **RosterWizard:** Saved a real position ("סיור") via "המשך למשימה הבאה". Clicked "הסר משימה זו" on it — `ConfirmDialog` opened correctly: "למחוק את 'סיור'? העמדה תימחק לצמיתות, כולל שיבוצים שכבר בוצעו לה השבוע הנוכחי (אם יש). הפעולה לא הפיכה." Cancel left it unchanged (tab still present). Confirming deleted it permanently — the position reverted to a draft seed suggestion, and a direct SQL query against `gs_positions` confirmed zero rows for that team.
  2. **PositionsScreen:** Re-saved the same position, navigated to "עוד" → "עמדות קבע", clicked "מחק את סיור" — identical `ConfirmDialog` copy opened. Confirming deleted it, screen reverted to "אין עדיין עמדות קבועות" (no positions), and SQL confirmed `gs_positions` count = 0 for the team.
  - The FK-safety-net scenario (deleting a position with a materialized current week) was not separately exercised in this pass since the test position had no assignments yet, but the underlying `unmaterializePositionWeek`-then-`deletePosition` ordering from Phase 11 is untouched by this plan's changes (only the UI call sites were gated) and was already live-verified in Phase 11's own closing pass.
