...
/**
 * MORE-01 — טבלת הביקורת (Phase 10, 10-CONTEXT.md → <decisions>).
 *
 * ביקורת מלאה, מבוססת קוד (לא השערה), על חמשת פריטי "עוד" *המקוריים* —
 * לפני שהתוכניות 10-01/10-02 יישמו את המסקנות שלה. הטבלה נשארת כאן, צמודה
 * ל-moreItems() עצמו, כי היא מתעדת למה המצב הנוכחי (4 פריטים, לא 5) הוא
 * הנכון — לא רק מה הוא.
 *
 * | פריט "עוד"                        | מיקום נוכחי (בזמן הביקורת)                                                                                    | כפול?                                                                                                  | המלצה                                                                                                      |
 * |-----------------------------------|----------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------|
 * | חילופים (swaps → SwapMgmt)        | "עוד"; כבר ב-Dashboard (StatCard "חילופים", views.jsx)                                                        | לא — יישום יחיד, קיצור כבר קיים                                                                          | MORE-02 כבר מתקיים. לשמור את שתי נקודות הכניסה.                                                              |
 * | משימות (tasks → TaskMgmt)         | "עוד"; כבר ב-Dashboard (StatCard "משימות פתוחות", views.jsx)                                                  | לא — WeekFlow שלב 0 לעולם לא יוצר kind="task"; TaskMgmt הוא המסך היחיד                                   | MORE-02 כבר מתקיים. לשמור.                                                                                    |
 * | עמדות קבועות (positions → PositionsScreen) | "עוד" בלבד. CRUD נגיש גם דרך RosterWizard (מצב army בלבד)                                              | חלקי — חפיפת CRUD רק במצב army, בכוונה מתועדת (RosterWizard); רשימת-כשירות ותחזית-4-שבועות ב-PositionsScreen ייחודיות | לא מוסרים (תוכן ייחודי שורד). לא מוסיפים קיצור-דרך — PositionsScreen.jsx עצמו מתעד שזה מסך שימוש-נמוך בכוונה. |
 * | מבט משאבים (resources → ResourceView) | "עוד" בלבד. אותו buildResourceRows+ResourceGrid בדיוק גם ב-CalendarView (מצב "week", ברירת המחדל) וגם ב-RosterWizard (army, שלב 0) | כן — כפילות מלאה מול CalendarView במצב השבועי שלו                                                        | להסיר מ"עוד" (MORE-03). "יומן" כבר מכסה את זה זהה, בלי אובדן פונקציונליות.                                    |
 * | דוחות/ניתוח (analytics → AnalyticsDash) | "עוד" בלבד. כרטיס "עומס" ב-Dashboard (views.jsx) מציג תת-קבוצה קלה מאותו מקור-נתונים, בלי קישור הלאה    | לא — ל-Analytics גרפים/טבלה/תגי-הוגנות ייחודיים; כרטיס ה-Dashboard הוא "טיזר" בלי המשך                    | להוסיף קיצור-דרך (MORE-02) — קישור "לדוח המלא" על כרטיס "עומס". משאירים גם ב"עוד".                            |
 *
 * המסקנה התפעולית מהטבלה, כפי שיושמה בפועל:
 * - MORE-03 (הסרה, 10-02-PLAN.md): resources הוסר מ-moreItems() ומ-views map.
 *   ResourceView.jsx נמחק כקוד מת (ResourceGrid/buildResourceRows ממשיכים
 *   לשרת CalendarView/RosterWizard — לא נגעו בהם).
 * - MORE-02 (הוספה, 10-01-PLAN.md): קיצור-דרך חדש אחד בלבד — analytics, כקישור
 *   "לדוח המלא" על כרטיס "עומס" הקיים ב-SupDashboard. swaps/tasks כבר היו
 *   להם קיצורים קיימים (StatCard) — רק אומתו מחדש, לא נבנו מחדש.
 * - positions לא קיבל קיצור-דרך חדש — החלטה מכוונת (10-CONTEXT.md), לא פריט
 *   שנשכח: זה המסך היחיד בלי קיצור, אבל הוא גם המסך היחיד שמוגדר פעם אחת
 *   וחוזר לבד — הוספת קיצור הייתה מנוגדת לתבנית השימוש המתועדת שלו.
 */
...
export default function SupervisorApp({ state }) {
  ...
  const startDemo = async (guardCount) => {
