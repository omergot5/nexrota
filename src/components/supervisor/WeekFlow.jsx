...
//
// כאן זה מסך אחד עם חמישה שלבים: ראשון בניית השבוע/הסד"כ, ומיד אחריו הלוח
// המאוחד (Phase 5, D-04) — שעבר למיקום הזה ב-Phase 8 (WEEKBUILD-02,
// 2026-09-22, ר' תיעוד STEP_OF למטה) — ואחריהם שלושת השלבים המקוריים
// הנותרים. פס השלבים הוא גם הניווט וגם הסטטוס: כל שלב מציג את המספר שלו
// (כמה משמרות, כמה פריטים על הלוח, כמה הגישו, כמה מאוישות, כמה פורסמו),
// אז המשתמש רואה איפה הוא עומד בלי לפתוח כלום.
//
...
export default function WeekFlow({
  ...
  const body = [
    ...
    // הלוח יושב עכשיו בשלב 1, מיד אחרי בניית שבוע (WEEKBUILD-02, Phase 8,
    // 2026-09-22) — במקום לפניה כפי שנקבע ב-Phase 5 (D-04). ברירת המחדל
    // הכללית של UnifiedBoard עדיין לא קוראת בשם שום כפתור (D-15), כי היא גם
    // מוצגת ביומן ואצל המשתתף — מסך בלי הפעולה הזו עליו. כאן, בתוך
    // WeekFlow, מצב-הריק המותאם עדיין מותר לקרוא בשם מפורש — אבל בשם השלב
