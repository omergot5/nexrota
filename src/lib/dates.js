 ...
 * נכונה — אם ההנחה הזו אי-פעם תישבר (שעה לא מרופדת, פורמט אחר), המיון
 * כאן ישקוט בשקט על סדר שגוי.
 *
 * `startTime` ריק/חסר נדחף לסוף (לא זורק), ושני פריטים באותה שעה נשברים
 * לפי `id` — אותו דפוס בדיוק שכבר קיים ב-`resourceView.js` — כדי שסדר
 * התיקו יהיה יציב וזהה בכל ריצה (עקרון ברזל 1: דטרמיניזם).
 */
...
export function taskAsShiftShape(task) {
  ...
  return {
    ...
    // D-01: תיקיית המשימה (`gs_tasks.category`, כבר קיימת מפאזה 2) *היא*
    // שדה הכשירות שלה — הגשר הזה מה שמאפשר ל-checkQualification (פאזה 3)
    // לשפוט משימה באותו קוד ששופט משמרת, בלי מקרה מיוחד. גשר ששותק על השדה
    // הזה הוא בדיוק הנקודה העיוורת שהמחזור הזה קיים כדי לסגור — והיא הייתה
    // נשארת בלתי-נראית עד שמישהו, פאזה אחת אחר-כך, היה תוהה למה משימה אף
    // פעם לא חוסמת. נופל למחרוזת ריקה כמו taskFromRow (api.js) לאותו שדה.
