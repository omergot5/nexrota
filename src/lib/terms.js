 ...
 * מיוצא (לא רק לשימוש פנימי) כדי ש-scripts/verify-terms.mjs יוכל לוודא
 * ש-PROFILE_TERMS לא דורס מפתח שלא קיים כאן — מפתח כזה היה מוצג כמו שהוא
 * (raw key, ר' `t()` למטה) בלי אזהרה, כי הדריסה עצמה לא הייתה נכנסת ל-merge.
 */
export const BASE = {
